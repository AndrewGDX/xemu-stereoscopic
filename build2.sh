...
show_help() {
    cat <<EOF
Usage: ./build2.sh [options] [configure options]
...
package_macos() {
    ...
    codesign --force --deep --preserve-metadata=entitlements,requirements,flags,runtime \
        --sign - "${exe_path}"
    PKG_CONFIG_LIBDIR="${configure_env_pkg_config_libdir}" \
...
needs_configure() {
    ...
    if any_newer_than "${build_dir}/config-host.mak" \
        ...
        "${project_source_dir}/build2.sh" \
        "${project_source_dir}/scripts/download-macos-libs.py" \
    ...
    temp_signature="$(mktemp "${TMPDIR:-/tmp}/build2-config.XXXXXX")"
...
config_signature_file="${build_dir}/.build2-config"
...
case "${platform}" in
    ...
    Darwin)
        ...
        append_flag configure_env_ldflags "-L${lib_prefix}/lib"
